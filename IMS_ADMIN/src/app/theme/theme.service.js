        ...
        serv.AuthService =
                {
                    ...
                    login: function (username, password)
                    {
                        ...
                        return $http({
                            method: 'POST', url: $rootScope.baseUrl + 'auth/signin', data: {
                                email: username,
                                password: password
                            }
                        }).then(function success(loginRes) {
                            loginRes = loginRes.data;
                            console.log("loginRes --",loginRes,username)
                            if (loginRes.email === username) {
                                if (loginRes.isPwdChanged) {
                                    $rootScope.$broadcast('isPwdChanged', true, false);
                                }
                                serv.AuthService.loggedIn = true;
                                $rootScope.userData = loginRes;
                                $rootScope.username = loginRes.name ? loginRes.name : loginRes.fName;
